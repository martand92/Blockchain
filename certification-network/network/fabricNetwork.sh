...
function checkPrereqs() {
  ...
  DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-tools:"1.4.9" peer version | sed -ne 's/ Version: //p' | head -1)
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v "$PWD":/tmp/certificationchannel --rm hyperledger/fabric-tools:"1.4.9" rm -Rf /tmp/certificationchannel/ledgers-backup
